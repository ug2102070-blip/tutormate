      ...
      {loading ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          {[1, 2, 3].map((i) => (
            <div
              key={i}
              className="h-44 rounded-2xl animate-shimmer border border-slate-200 bg-white"
            />
          ))}
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          {filteredBatches.map((batch) => (
            <div
              key={batch.id}
              className="p-6 rounded-2xl border border-slate-200 bg-white flex flex-col justify-between transition-all duration-200 hover:shadow-md shadow-xs"
            >
              <div>
                <div className="flex items-start justify-between gap-2">
                  <div>
                    ...
                    <div className="flex items-center gap-2 mt-1.5">
                      <span className="px-2.5 py-0.5 text-[11px] font-semibold rounded-full bg-slate-100 text-slate-700">
                        {batch.subject}
                      </span>
                      <span className="px-2.5 py-0.5 text-[11px] font-semibold rounded-full bg-indigo-50 text-indigo-700 border border-indigo-100">
                ...
                <div className="mt-4 pt-4 border-t border-slate-100 space-y-2">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {batch.schedule?.map((s, idx) => (
                      <span
                        key={idx}
                        className="text-[11px] px-2.5 py-1 rounded-lg font-semibold bg-slate-50 text-slate-700 border border-slate-200/60"
                      >
                        {s.day}: {s.time}
                      </span>
                    ))}
