...
export default function StudentAssignmentDetailsPage() {
  ...
  if (!assignment || !submission) {
    return (
      <div className="text-center py-12">
        ...
        <p className="text-slate-500 mt-2">The assignment you're looking for doesn't exist or you don't have access to it.</p>
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <Link href="/student/assignments" className="inline-flex items-center text-sm font-medium text-slate-500 hover:text-indigo-600 transition-colors">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm">
        <div className="flex justify-between items-start mb-4">
          <div>
            <div className="flex items-center gap-3 mb-2">
              ...
              {submission.status === 'graded' && <span className="inline-flex items-center gap-1 text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded text-[10px] font-bold uppercase border border-emerald-200"><CheckCircle className="w-3 h-3" /> Graded</span>}
            </div>
            <p className={`flex items-center text-sm font-medium ${isOverdue ? 'text-red-500' : 'text-slate-500'}`}>
        ...
        {assignment.description && (
          <div className="mt-6">
            ...
            <div className="text-slate-700 bg-slate-50 p-4 rounded-xl text-sm border border-slate-100 whitespace-pre-wrap">
              {assignment.description}
            </div>
          </div>
        )}
        ...
        {submission.status === 'graded' && submission.feedback && (
          <div className="mt-6">
            <h3 className="text-sm font-bold text-slate-800 mb-2">Tutor Feedback</h3>
            <div className="text-indigo-900 bg-indigo-50 p-4 rounded-xl text-sm border border-indigo-100 italic">
              "{submission.feedback}"
            </div>
          </div>
        )}
      ...
      <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm">
        ...
        {submission.status === 'pending' ? (
          <form onSubmit={handleSubmit}>
            ...
            {!file ? (
              <div 
                className="border-2 border-dashed border-slate-200 rounded-xl p-8 flex flex-col items-center justify-center text-center cursor-pointer hover:border-indigo-400 hover:bg-indigo-50/50 transition-colors"
                onClick={() => fileInputRef.current?.click()}
              >
                <div className="w-12 h-12 bg-indigo-50 rounded-full flex items-center justify-center mb-3">
                  <Upload className="w-6 h-6 text-indigo-500" />
                </div>
                <h4 className="text-sm font-bold text-slate-700 mb-1">Click to upload your assignment</h4>
                <p className="text-xs text-slate-500 max-w-xs">PDF, Word, Images, or Video files accepted. Make sure your file is clear and readable.</p>
                <input 
                  type="file" 
                  ref={fileInputRef} 
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between p-4 bg-slate-50 border border-slate-200 rounded-xl gap-4">
            <div className="flex items-center gap-4 text-left w-full sm:w-auto">
              ...
              <div>
                <p className="text-sm font-bold text-slate-800">Assignment Submitted</p>
            ...
            {submission.filePath && (
              <button
                ...
                className="w-full sm:w-auto flex items-center justify-center gap-2 py-2 px-4 bg-white border border-slate-200 hover:bg-slate-50 text-slate-700 text-sm font-semibold rounded-lg transition-colors shadow-sm"
              >
