...
export default function StudentExamsPage() {
  ...
  if (error) {
    return (
      <div className="max-w-4xl mx-auto p-6 bg-red-50 border border-red-200 rounded-2xl">
    ...
    <div className="max-w-4xl mx-auto space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1">Track your performance and grades across all batches.</p>
        </div>
      </div>

      {results.length === 0 ? (
        <div className="text-center p-12 bg-white rounded-2xl border border-slate-200 shadow-xs mt-6">
        ...
        <div className="grid gap-6">
          {results.map(({ result, exam }) => {
            ...
            return (
              <div key={result.id} className="bg-white rounded-2xl border border-slate-200 p-6 shadow-xs overflow-hidden relative">
                {/* Header info */}
                <div className="flex flex-wrap justify-between items-start gap-4 mb-6">
                  ...
                  {result.isAbsent ? (
                    <div className="bg-red-50 text-red-700 px-4 py-2 rounded-xl border border-red-100 font-bold tracking-wide">
                      ABSENT
                    </div>
                  ) : result.grade ? (
                    <div className={`flex items-center gap-3 px-5 py-3 rounded-2xl border ${
                      result.grade.includes('A') ? 'bg-emerald-50 border-emerald-100 text-emerald-800' :
                      result.grade === 'F' ? 'bg-red-50 border-red-100 text-red-800' :
                    ...
                    <div className="bg-slate-50 text-slate-500 px-4 py-2 rounded-xl border border-slate-200 font-semibold text-sm">
                      Pending Grade
                    </div>
                  )}
                </div>
                ...
                {!result.isAbsent && (
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4 bg-slate-50 rounded-2xl p-4 border border-slate-100">
                    <div className="bg-white rounded-xl p-3 shadow-xs border border-slate-100">
                      <p className="text-xs font-semibold text-slate-500 uppercase mb-1">Marks Obtained</p>
                    ...
                    <div className="bg-white rounded-xl p-3 shadow-xs border border-slate-100">
                      <p className="text-xs font-semibold text-slate-500 uppercase mb-1">Percentage</p>
                      <p className="text-lg font-bold text-slate-900">
                    ...
                    <div className="bg-white rounded-xl p-3 shadow-xs border border-slate-100">
                      <p className="text-xs font-semibold text-slate-500 uppercase mb-1">Batch Rank</p>
                ...
                {/* Remarks Section */}
                {result.remarks && (
                  <div className="mt-4 p-4 rounded-xl bg-amber-50 border border-amber-100 text-amber-900">
                    <p className="text-xs font-bold uppercase tracking-wider mb-1 text-amber-700">Tutor Remarks</p>
